...
for i in range(0, 20):
    for j in range(0, 17):
        value = int(grid_array[i * 20 + j]) * int(grid_array[i * 20 + j + 1]) * int(grid_array[i * 20 + j + 2])
        value *= int(grid_array[i * 20 + j + 3])
        if value > result:
            result = value
...
for i in range(0, 17):
    for j in range(0, 20):
        value = int(grid_array[i * 20 + j]) * int(grid_array[(i + 1) * 20 + j]) * int(grid_array[(i + 2) * 20 + j])
        value *= int(grid_array[(i + 3) * 20 + j])
        if value > result:
            result = value
...
for i in range(0, 17):
    for j in range(0, 17):
        value = int(grid_array[i * 20 + j]) * int(grid_array[(i + 1) * 20 + j + 1])
        value *= int(grid_array[(i + 2) * 20 + j + 2]) * int(grid_array[(i + 3) * 20 + j + 3])
        if value > result:
            result = value
...
for i in range(3, 20):
    for j in range(3, 20):
        value = int(grid_array[(i - 3) * 20 + j]) * int(grid_array[(i - 2) * 20 + j - 1])
        value *= int(grid_array[(i - 1) * 20 + j - 2]) * int(grid_array[i * 20 + j - 3])
        if value > result:
            result = value
